...
const PlayerResultDisplay = ({ playerResult }: { playerResult: PlayerResult }) => (
    ...
    >
        ...
        {/* --- REMOVED: Penalty display is no longer shown here --- */}
...
export const MatchResultOverlay: React.FC<MatchResultOverlayProps> = ({ result, currentUserEmail }) => {
  ...
  return (
    ...
    >
      ...
      <motion.div 
        // NOTE: I've added a padding-bottom adjustment (pb-12) to make it look better without the button.
        className={`bg-zinc-900 border-2 ${outcomeConfig.borderColor} ${outcomeConfig.shadowColor} rounded-xl p-8 pb-12 max-w-lg w-full text-center`}
      ...
      >
        ...
        <motion.div 
            className="grid grid-cols-2 gap-4" // Removed mb-8 as the button is gone
